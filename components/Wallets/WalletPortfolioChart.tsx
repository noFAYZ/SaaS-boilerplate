...
import { zerionUtils } from '@/lib/zerion';
import { HugeiconsAnalyticsUp } from '../icons/icons';
...
const WalletPortfolioChart: React.FC<PortfolioChartProps> = ({ 
  ...
  // Fetch data effect using zerion-service
  useEffect(() => {
    const fetchChartData = async () => {
      setIsLoading(true);
      
      try {
        // Use zerionUtils to fetch real chart data
        const chartPoints = await zerionUtils.getWalletChart(walletAddress, period);
        console.log('Fetched chart points:', chartPoints);
        
        if (chartPoints && Array.isArray(chartPoints?.attributes?.points) && chartPoints?.attributes?.points?.length > 0) {
          // Process real Zerion data
          const processedData: ChartPoint[] = chartPoints?.attributes?.points?.map((point: any) => ({
            timestamp: point[0],
            value: point[1] || 0
          }));
        ...
        // Generate fallback data if Zerion fails
    ...
    const generateFallbackData = (): ChartPoint[] => {
      const pointCount = period === 'day' ? 24 : 
                        period === 'week' ? 7 : 
                        period === 'month' ? 30 : 
                        period === 'year' ? 12 : 24;
      
      const baseValue = 12000 + Math.random() * 8000;
      const trendMultiplier = Math.random() > 0.3 ? 1 : -1;
      const volatility = baseValue * 0.15;
      
      return Array.from({ length: pointCount }, (_, i) => {
        const progress = i / (pointCount - 1);
        const trendComponent = trendMultiplier * progress * (baseValue * 0.2);
        const randomComponent = (Math.random() - 0.5) * 2 * volatility;
        
        return {
          timestamp: Date.now() - (pointCount - i) * getPeriodTimeUnit(),
          value: Math.max(0, baseValue + trendComponent + randomComponent)
        };
      });
    };
